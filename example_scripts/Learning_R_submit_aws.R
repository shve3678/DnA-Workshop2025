...
workdir <- '/scratch/Users/shve3678/workshop-day6/results/'
setwd(workdir)
getwd()
...
library(ggplot2)
h_line=15
g_combined <- ggplot(mtcars, aes(wt, mpg, color=cyl))+
  ...
  theme_minimal() +
  ylim(c(10,34))+xlim(c(3,9))+ 
  geom_hline(yintercept=h_line, linetype="dashed", color = "black") +
  geom_text(x=2.1, y=0.8, label="Efficient cars", color='#2540D8')

ggsave("end_cars.jpg", plot = last_plot(), path = workdir,
